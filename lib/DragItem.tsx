...
const DragItem = ({children}) => {
    const origin = {
        x: useSharedValue(SCREEN_WIDTH - CIRLE_SIZE - gapX * 2),
        y: useSharedValue(0),
    };

    useEffect(() => {
        (async () => {
            const data = await AsyncStorage.getItem('appConsoleDragItem');
            if (data) {
    ...
    const translateX = useSharedValue(SCREEN_WIDTH - CIRLE_SIZE - gapX * 2);
